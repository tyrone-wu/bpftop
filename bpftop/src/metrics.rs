...
/// Compile metrics into a list from the OpenMetrics text format.
pub(crate) fn deserialize(buffer: &str) -> IResult<&str, impl Iterator<Item = BpfProgram>> {
    // OpenMetrics text format is represented as a columnstore. Since the order of emitted metrics is
    // non-deterministic, we collect and compile together based on ID.
...
/// Parses an entire metric section, starting with the metadata portion and then the metrics portion.
